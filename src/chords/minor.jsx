...
export default function MinorChords() {
  const minorChords = [
    { name: "A Minor", shape: [null, 0, 2, 2, 1, 0] },   // x02210
    { name: "E Minor", shape: [0, 2, 2, 0, 0, 0] },       // 022000
    { name: "D Minor", shape: [null, null, 0, 2, 3, 1] }, // xx0231
    { name: "C Minor (Barre)", shape: [3, 5, 5, 4, 3, 3] }, // Cm barre
    { name: "G Minor (Barre)", shape: [3, 5, 5, 3, 3, 3] }, // Gm barre
  ];

  const [currentIndex, setCurrentIndex] = useState(0);
  const currentChord = minorChords[currentIndex];

  const activePositions = fretboardNotes.filter((pos) =>
    currentChord.shape.some((fret, index) => {
      if (fret === null) return false;

      const stringNumber = index + 1; // same working logic
      return (
  ...
  const nextChord = () => {
    setCurrentIndex((prev) =>
      prev === minorChords.length - 1 ? 0 : prev + 1
    );
  };

  const prevChord = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? minorChords.length - 1 : prev - 1
    );
  };
